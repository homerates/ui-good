...
async function runResponseCoverageAndContractTests(
  ...
  // Currently property-intelligence-v1.5 (2026-09-10) -- a deliberate,
  // documented version bump (see outputSchema.ts's file header), most
  // recently adding property_analysis (Deep Intelligence Parity), not an
  // accidental drift. Update this literal alongside any future bump.
  record('O', 'contract_version is the current deliberate version', 'property-intelligence-v1.5', rNa.ok ? rNa.data.contract_version : 'n/a', rNa.ok && rNa.data.contract_version === 'property-intelligence-v1.5' ? 'PASS' : 'FAIL', '');
