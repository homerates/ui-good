...
async function runResponseCoverageAndContractTests(
  ...
  // Currently property-intelligence-v1.7 (2026-09-14) -- a deliberate,
  // documented version bump (see outputSchema.ts's file header), most
  // recently adding original_list_price (Price History grounding), not an
  // accidental drift. Update this literal alongside any future bump.
  record('O', 'contract_version is the current deliberate version', 'property-intelligence-v1.7', rNa.ok ? rNa.data.contract_version : 'n/a', rNa.ok && rNa.data.contract_version === 'property-intelligence-v1.7' ? 'PASS' : 'FAIL', '');
