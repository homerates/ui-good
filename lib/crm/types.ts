...
// ── Platform-event fact types (Phase 2 auto-capture) ─────────────────────────
// Generated by the platform itself (AMI qualifier runs, affordability scenarios,
// property views) — not by human input. No D7 blocklist check needed (no
// free-form text about protected characteristics). D1 compliance enforced at
// the type level — no income/credit/DTI fields permitted here.

export type AmiResultFact = {
  key: 'ami_result';
  county: string;
  state: string;
  home_ready: boolean;
  home_possible: boolean;
  dpa: boolean;
  dpa_program_count: number;
};

export type AffordabilityRunFact = {
  key: 'affordability_run';
