...
3. **Factor in closing costs** (~2-3% of loan = $${Math.round(result.loanAmount * 0.025).toLocaleString()})`;
    ...
    if (!hasFHAWithPrice && !isPureRateInfo && (isAffordabilityQuestion(question) || (affordFollowUp.isFollowUp && (priorAffordContext?.annualIncome || affordFollowUp.useCurrentRate)))) {
        ...
        if (affordParams.hasInfo) {
            ...
            if (isIncomeQuery && priorMonthlyPayment && priorCalcSnap) {
                // Answer income qualification question using prior scenario numbers
                const piti = Math.round(priorMonthlyPayment);
                const inc43 = Math.round((piti / 0.43) * 12);
                const inc36 = Math.round((piti / 0.36) * 12);
                const inc28 = Math.round((piti / 0.28) * 12);
                const scenarioDesc = snapshotLoanType?.replace('calcEngine-', '') ?? 'your loan';
                affordabilityAnswer = {
                    answer: `**Income needed to qualify for ${scenarioDesc} scenario**\n\nBased on your prior scenario with a monthly payment of **$${piti.toLocaleString()}**, here is the income required at standard DTI thresholds:\n\n| DTI threshold | Monthly income needed | Annual income needed |\n|---|---|---|\n| 43% (standard max) | $${Math.round(piti / 0.43).toLocaleString()} | **$${inc43.toLocaleString()}** |\n| 36% (conservative) | $${Math.round(piti / 0.36).toLocaleString()} | **$${inc36.toLocaleString()}** |\n| 28% (front-end only) | $${Math.round(piti / 0.28).toLocaleString()} | **$${inc28.toLocaleString()}** |\n\n*Assumes no other monthly debts. Each $500/mo in existing debt (car, student loans) adds ~$14k to the annual income requirement at 43% DTI.*`,
                    ...
                    follow_up_chips: [
                        ...
                        { label: 'Run full affordability instead', seed: `How much home can I afford?` },
                    ],
