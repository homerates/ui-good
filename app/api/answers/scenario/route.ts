...
function normalizeForGrokCard(result: any, message: string, marketData: any) {
    ...
    /* =========================
       FINAL HARD LOCK: Deterministic math wins
       Runs at the very end so nothing can overwrite it later.
    ========================= */
    try {
        const si: any = (out as any).scenario_inputs || (out as any).scenarioInputs || null;

        // Local formatters (avoid dependency on file-level helpers)
        ...
        // Helper: read first finite number from a list of candidate keys
        ...
        // Normalize percent inputs: accept 5 or 0.05; output is PERCENT (5)
        const toPct = (v: any) => {
            const n = Number(v);
            if (!Number.isFinite(n) || n < 0) return 0;
            return n > 0 && n < 1 ? n * 100 : n;
        ...
        // 1) Monthly P&I: derive from amort Year 1 totals if present
        let monthlyPI = Number(out.monthly_payment);
        const y1 =
            Array.isArray(out.amortization_summary) && out.amortization_summary.length
                ? out.amortization_summary[0]
                : null;
        ...
        // 2) Pull core inputs with robust key fallbacks
        const price = pickNum(si, ["purchase_price", "purchasePrice", "price", "property_value"], NaN);

        // Rent is the main issue. Try common names.
        const rent = pickNum(
            si,
            [
                "rent_monthly",
        ...
        const vacancyPct = toPct(pickNum(si, ["vacancy_pct", "vacancy", "vacancyPct"], 0));
        const maintPct = toPct(pickNum(si, ["maintenance_pct", "maintenance", "maintenancePct"], 0));
        const taxPct = toPct(pickNum(si, ["tax_pct", "property_tax_pct", "taxPct"], 0));
        const insPct = toPct(pickNum(si, ["insurance_pct", "insurance", "insurancePct"], 0));

        // HOA is monthly dollars only
        const hoa = pickNum(si, ["hoa_monthly", "hoa_monthly_amount", "hoa", "monthly_hoa"], 0);

        // 3) Deterministic monthly components
        const monthlyTax =
            Number.isFinite(price) && Number.isFinite(taxPct) ? (price * (taxPct / 100)) / 12 : 0;
        ...
        const pitia = monthlyPI + monthlyTax + monthlyIns + (Number.isFinite(hoa) ? hoa : 0);

        const effectiveRent =
            (Number.isFinite(rent) ? rent : 0) * (1 - (Number.isFinite(vacancyPct) ? vacancyPct / 100 : 0));
        ...
        const dscrGross = Number.isFinite(rent) && rent > 0 && pitia > 0 ? rent / pitia : null;
        const dscrEffective = effectiveRent > 0 && pitia > 0 ? effectiveRent / pitia : null;
        ...
        (out as any).computed_financials = {
            ...
            monthly_cash_flow: monthlyCashFlow,
            annual_cash_flow: annualCashFlow,
            debug_rent_used: rent,
            debug_price_used: price,
        };

        // Force flat cash flow table (no growth)
        if (Array.isArray(out.cash_flow_table) && out.cash_flow_table.length) {
            out.cash_flow_table = out.cash_flow_table.map((row: any) => ({
                year: row.year,
                net_cash_flow: annualCashFlow,
        ...
        // Overwrite the summary so it cannot lie about payment and cash flow
        if (Number.isFinite(monthlyPI) && Number.isFinite(pitia)) {
            (out as any).plain_english_summary =
                `Deterministic math: monthly P&I ${money2(monthlyPI)}. ` +
                `Monthly PITIA ${money2(pitia)} (tax ${money2(monthlyTax)}, ins ${money2(monthlyIns)}, HOA ${money2(hoa)}). ` +
                `Rent used ${money2(rent)}. Effective rent ${money2(effectiveRent)}. ` +
                `DSCR gross ${pct2(dscrGross)}, DSCR effective ${pct2(dscrEffective)}. ` +
                `Monthly cash flow ${money2(monthlyCashFlow)}. Annual cash flow ${money2(annualCashFlow)}.`;
        }
    } catch { }
