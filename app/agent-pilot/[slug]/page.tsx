"use client";
// app/agent-pilot/[slug]/page.tsx
// Agent pilot landing page — sent to principal brokers / office managers via outreach.
...
export default function AgentPilotPage() {
  ...
  useEffect(() => {
    if (!slug) return;
    fetch(`/api/agent-pilot/${slug}`)
      .then(r => {
        if (!r.ok) { setNotFound(true); setLoading(false); return null; }
        return r.json();
      })
      .then(d => { if (d) setPilot(d); setLoading(false); })
      .catch(() => { setNotFound(true); setLoading(false); });
  }, [slug]);

  const activateHref = `/welcome?pilot=${slug}&pilot_type=agent&role=agent`;
    ...
    <div style={styles.root}>
      ...
      <div style={styles.container}>
        <div style={styles.card}>
          ...
          {/* Eyebrow */}
          <div style={styles.eyebrow}>Free Agent Pilot Access</div>
          ...
          <h1 style={styles.headline}>
            AI home + mortgage intelligence<br />for your agents.
          </h1>

          <p style={styles.sub}>
            The First AI Home + Mortgage Intelligence Platform. Live market data, buyer decision scoring, property intelligence, and financing scenarios — built for agents who win clients with data, not guesswork.
          </p>

          {/* Benefits */}
          <div style={styles.benefits}>
            {[
              { icon: "⚡", label: `${pilot.credits_per_lo.toLocaleString()} AI analysis credits`, sub: "Run full property and buyer scenarios for multiple clients" },
              { icon: "🏅", label: "Founding Member", sub: "Permanently on your profile — visible to every buyer you work with" },
              { icon: "🏠", label: "Property intelligence + buyer scoring", sub: "Decision Score, market comps, affordability — in one place" },
              { icon: "🎯", label: "Direct line to the founders", sub: "Shape the product from day one for agents" },
            ].map(b => (
...
const styles: Record<string, React.CSSProperties> = {
  ...
  navSignIn: { fontSize: "0.82rem", color: "#8fa3b8", textDecoration: "none" },
  container: {
  ...
  companyRow: { display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" as const },
  companyName: { fontSize: "1.1rem", fontWeight: 800, color: "#f0f4ff" },
  cross: { fontSize: "1rem", color: "#8fa3b8" },
  brand: { fontSize: "1.1rem", fontWeight: 800, color: "#00e87a" },
  headline: {
    fontSize: "1.75rem", fontWeight: 800, margin: 0,
    lineHeight: 1.2, letterSpacing: "-0.02em", color: "#f0f4ff",
  },
  sub: { fontSize: "0.9rem", color: "#8fa3b8", margin: 0, lineHeight: 1.6 },
  benefits: { display: "flex", flexDirection: "column" as const, gap: 12 },
  benefit: {
  ...
  trust: { textAlign: "center" as const, fontSize: "0.78rem", color: "rgba(143,163,184,0.6)" },
  footer: { textAlign: "center" as const, fontSize: "0.75rem", color: "#8fa3b8" },
  secondaryBtn: {
...
const css = `
  @media (max-width: 480px) {
    .agent-pilot-card { padding: 1.75rem !important; }
  }
`;
