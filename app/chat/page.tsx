...
function isPaymentQuery(q: string) {
    ...
    // "$500k with 20% down at 6.25% for 30 years"
    if (/\$\s*\d[\d.,]*\s*(k|m)?\s+with\s+\d+(\.\d+)?\s*%\s*down.*\bat\s+\d+(\.\d+)?\s*%\s+for\s+\d+/.test(s)) return true;

    // "loan 400k at 6.5% for 30y"
    if (/\bloan\b.*\bat\s+\d+(\.\d+)?\s*%\s+for\s+\d+/.test(s)) return true;
...
export default function Page() {
    ...
    // Unified calc runner used by both typed queries and the Mortgage Calculator overlay
    async function runCalc(params: {
        loanAmount?: number;
        purchasePrice?: number;
        downPercent?: number;
        annualRatePct?: number;
    ...
    async function send() {
        ...
        try {
            if (isPaymentQuery(q)) {
                ...
                await runCalc({
                    loanAmount: patched.loanAmount,
                    purchasePrice: patched.purchasePrice,
                    downPercent: patched.downPercent,
                    annualRatePct: patched.annualRatePct,
                    termYears: patched.termYears,
                    rawQ: q,
                });
            ...
            const friendly =
                ...
                (meta.fred &&
                    ...
                    : typeof meta.answer === 'string'
                        ? meta.answer
                        : `path: ${meta.path} | usedFRED: ${String(meta.usedFRED)}`);
                    ...
                    >
                        ...
                        >
                            <input
                                ...
                                onKeyDown={onKey}
                                // leave the look to your existing .input styles; just reserve space for the button
                                style={{
                                    flex: '1 1 auto',
                                    minWidth: 0,
                                    paddingRight: 52,         // room for the circular send button
                                    borderRadius: 12,         // keep your existing visual language
                                }}
                            />
                            ...
                            <button
                                ...
                                // fixed-size circular button, positioned inside the input on the right
                                style={{
                                    position: 'absolute',
                                    right: 6,
                                    top: '50%',
                                    ...
                                    // keep your existing btn colors; this only ensures it stays a circle
                                    flex: '0 0 38px',
                                }}
                            >
                                {/* simple arrow, no imports */}
                                <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
                                    <path d="M3 12h14.5M13 6l6 6-6 6" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                                </svg>
                            </button>
                        </div>
                    </div>
