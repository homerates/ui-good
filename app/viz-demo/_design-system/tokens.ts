...
//
// Ported from the standalone viz-module/ Vite scaffold into this Next.js route. Components use
// these values directly (Recharts/Framer Motion take style/props objects, not Tailwind classes)
// and Tailwind classes elsewhere use literal hex arbitrary values matching these numbers —
// deliberately NOT registered in the app's shared Tailwind theme, so this demo page can't leak
// a `brand`/`neutral` palette into the rest of the (dark-themed) app.

// `brand.600` (#00e87a) is HomeRates' actual accent color — matches `var(--accent)` in
// app/globals.css and COLORS.accent in lib/tokens.ts.
export const colors = {
  brand: {
    50: '#e8fdf3',
    100: '#c3fadf',
