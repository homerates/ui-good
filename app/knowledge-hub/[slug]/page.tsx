...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      ...
      type: 'article',
      images: [{ url: `https://chat.homerates.ai/api/og?title=${encodeURIComponent(article.title)}&cat=knowledge-hub`, width: 1200, height: 630 }],
    },
    twitter: { card: 'summary_large_image', title: article.title, description: article.excerpt, images: [`https://chat.homerates.ai/api/og?title=${encodeURIComponent(article.title)}&cat=knowledge-hub`] },
  };
}

/** Minimal markdown → HTML converter (headers, bold, tables, lists, hr) */
...
export default async function KnowledgeHubArticle({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  return (
    <>
      <style>{`
        ...
        .kha-root * { box-sizing: border-box; }
        body:has(.kha-root) .app-footer { display: none; }
        ...
        .kha-nav-logo { display: flex; align-items: center; text-decoration: none; }
        .kha-nav-logo img { height: 32px; width: auto; }
        .kha-nav-back {
          display: inline-flex; align-items: center; gap: 6px;
          font-size: 13px; color: var(--text-muted);
          text-decoration: none; transition: color 0.2s;
        }
        .kha-nav-back:hover { color: var(--text); }
        ...
        .kha-body tr:nth-child(even) td { background: rgba(255,255,255,0.02); }
        ...
        .kha-related-grid { display: flex; flex-direction: column; gap: 12px; }
        .kha-related-card {
          background: var(--surface);
          border: 1px solid var(--border);
        ...
        .kha-related-card:hover .kha-related-arrow { transform: translateX(4px); color: var(--green); }
        ...
        .kha-footer-links { display: flex; gap: 20px; }
        .kha-footer-links a { color: var(--text-dim); text-decoration: none; transition: color 0.2s; }
        .kha-footer-links a:hover { color: var(--text-muted); }

        @media (max-width: 768px) {
          .kha-nav { padding: 14px 20px; }
          .kha-wrap { padding: 36px 20px 60px; }
          .kha-wrap h1 { font-size: 28px; }
          .kha-excerpt { font-size: 16px; }
          .kha-related { padding: 0 20px 60px; }
          .kha-footer { padding: 24px 20px 40px; flex-direction: column; gap: 16px; }
        }
      `}</style>

      <div className="kha-root">
        {/* NAV */}
        ...
        {/* ARTICLE */}
        <div className="kha-wrap">
          <div className="kha-cat" style={{ color: catColor }}>
            <span style={{ width: 6, height: 6, background: catColor, borderRadius: '50%', display: 'inline-block' }} />
            {article.category}
          </div>
          <h1>{article.title}</h1>
          <p className="kha-excerpt">{article.excerpt}</p>
          <div className="kha-meta">
          ...
          <div className="kha-cta">
            ...
            <Link href="/chat">Try the Mortgage Calculator →</Link>
          </div>
        </div>

        {/* RELATED */}
        {related.length > 0 && (
          <div className="kha-related">
            <div className="kha-related-label">More in {article.category}</div>
            <div className="kha-related-grid">
              {related.map((r) => (
                <Link key={r.slug} href={`/knowledge-hub/${r.slug}`} className="kha-related-card">
                  <div>
                    <h4>{r.title}</h4>
                    <span>{r.date} · {r.readTime}</span>
                  </div>
                  <span className="kha-related-arrow">→</span>
                </Link>
              ))}
            </div>
          </div>
        ...
        <footer className="kha-footer">
          <div>HomeRates.ai · Educational content only · Not financial advice</div>
          <div className="kha-footer-links">
            <Link href="/knowledge-hub">All Articles</Link>
