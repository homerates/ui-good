...
type ApiResponse = {
  ...
  usedFRED: boolean;
  // NEW: make these part of the type so we don't cast to any
  message?: string;
  summary?: string;
...
export default function Page() {
  ...
  async function send() {
    ...
    try {
      ...
      // Friendly display logic without `any`
      function displayLine(j: ApiResponse): string {
        ...
        if (j.fred && j.fred.tenYearYield && j.fred.mort30Avg && j.fred.spread) {
          const f = j.fred;
          const y = typeof f.tenYearYield === 'number' ? f.tenYearYield.toFixed(2) : f.tenYearYield;
          const m = typeof f.mort30Avg === 'number' ? f.mort30Avg.toFixed(2) : f.mort30Avg;
          const s = typeof f.spread === 'number' ? f.spread.toFixed(2) : f.spread;
          return `As of ${f.asOf ?? 'recent data'}: 10Y ${y}%, 30Y ${m}%, spread ${s}%.`;
        }
        return `path: ${j.path} · usedFRED: ${String(j.usedFRED)} · confidence: ${j.confidence ?? '-'}`;
      }

      const line = displayLine(meta);

      setMessages((m) => [...m, { id: uid(), role:'assistant', content: line, meta }]);
    } catch (e) {
